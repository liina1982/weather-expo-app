...
interface ForecastControlType {
  onPress: (forecastType: ForecastType) => void
}
const ForecastControl = ({ onPress }: ForecastControlType): JSX.Element => {
  ...
  const onForecastPress = (type: ForecastType): void => {
    if (type === ForecastType.Weekly) {
      segmentTranslateX.value = withTiming(width - textWidth - spacingX * 2, { duration: 1000 })
    } else {
  ...
  return (
    ...
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: spacingX }}>
        <TouchableOpacity onPress={() => { onForecastPress(ForecastType.Hourly) }}>
            <Text onLayout={onTextLayout} style={styles.forecasText}>Hourly Forecast</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => { onForecastPress(ForecastType.Weekly) }}>
            <Text onLayout={onTextLayout} style={styles.forecasText}>Weekly Forecast</Text>
        </TouchableOpacity>
    </View>

    <AnimatedCanvas style={[{ height: strokeWidth, width: textWidth, marginLeft: spacingX }, animatedSegmentLineStyles]}>
