...
interface ForecastModalBackgroundProps {
    width: number,
    height: number,
    cornerRadius: number
}
const ForecastModalBackground = ({width, height, cornerRadius}: ForecastModalBackgroundProps) => {
    const borderPath = `M 0 ${cornerRadius}
                        A ${cornerRadius} ${cornerRadius} 0 0 1 ${cornerRadius} 0
                        H ${width-cornerRadius}
                        A ${cornerRadius} ${cornerRadius} 0 0 1 ${width} ${cornerRadius}`
  return (
    <BlurView style= {{...StyleSheet.absoluteFillObject, borderRadius: cornerRadius, overflow: 'hidden'}} intensity={50} tint='dark'>
      <Canvas style={{flex:1}}>
...
export default ForecastModalBackground
